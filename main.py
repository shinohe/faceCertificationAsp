...
import face_keras_mat as face_classify
...
# flaskでpredict呼べない問題
global model, model_classify, graph
...
logger.debug(image_size[0])
model_classify = face_classify.build_model((image_size[0], image_size[0], 3))
model_classify.load_weights("./data/face-gender-model.h5")
...
def classifyFacePredict(data):
	...
	try:
		...
		# flaskでpredict呼べない問題
		with graph.as_default():
			pre = model_classify.predict(X)[0].tolist()
...
@app.route('/faceClassify', methods=['GET'])
def faceClassify():
	# faceClassify.html をレンダリングする
	return render_template('faceClassify.html', isMenu=True)
...
def classifyFace():
	...
	if request.data:
		...
		if 'input' in content_body_dict:
			...
			predict = classifyFacePredict(inputData)
	
	return jsonify({'predict':predict,'labels':{'gender':gender.tolist(), 'age':age.tolist(), 'name':name}})
